def findNeighbors2(partition_data): 
  b_index = broadcasted_index.value b_vectors = broadcasted_vectors.value
...
index = faiss.IndexFlatL2(emb_size) # build the index print(index.is_trained) index.add(np.array(vectors['embedding'].tolist()).astype("float32")) # add vectors to the index vectors.columns = ['item_id','embedding'] vectors = vectors[['item_id']]
